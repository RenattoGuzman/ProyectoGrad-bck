...
from ..variables import INDUSTRIES_FILENAME, STOCKS_FILENAME, START_TRAINING_DATE, END_TRAINING_DATE, END_REALDATA_DATE
...
@router.get('/portfolio/min-variance')
def recommend_portfolio_min_variance(industries: List[str] = Query(...)):
...
@router.get('/portfolio/max-variance')
def recommend_portfolio_max_variance(industries: List[str] = Query(...)):
    filtered = industries_df[industries_df['sector'].isin(industries)]
    symbols = filtered['Symbol'].dropna().unique().tolist()
    df_filtered = stocks_df.loc[stocks_df.index.get_level_values('symbol').isin(symbols)]
    training_data = df_filtered[(df_filtered.index.get_level_values('date') >= START_TRAINING_DATE) &
    ...
    S = risk_models.CovarianceShrinkage(prices_clean).ledoit_wolf()
    ef = EfficientFrontier(mu, S)
    ef.efficient_risk(target_volatility=50.0) # Arbitrary high risk to get max volatility
    weights = ef.clean_weights()
    perf = ef.portfolio_performance()
    mega_cleaned_weights = mega_clean_weights(weights)
